...
from backend.config import SKIP_DIR_PATTERNS, SKIP_FILE_PATTERNS
...
def analyze_repository(repo_path, verbose=False, parallel=False, max_workers=4):
    ...
    parallel=True enables parallel file analysis (default: False for Windows compatibility)
    max_workers=4 sets the number of parallel workers (default: 4)
    
    NOTE: Parallel mode disabled by default on Windows due to multiprocessing limitations.
          Use ThreadPoolExecutor instead for Windows compatibility.
    """
    results      = []
    skipped_dirs  = []
    skipped_files = []
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for file in files:
            ...
            # Quick pre-filter: only queue files with supported extensions
            if get_language(file_path) is not None and not should_exclude(file_path):

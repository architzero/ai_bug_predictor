...
from backend.config import CHECKPOINT_DIR as _CHECKPOINT_DIR, MINER_CACHE_DIR as _MINER_CACHE_DIR, SZZ_CACHE_DIR
...
from backend.analysis import SUPPORTED_EXTENSIONS
...
def mine_git_data(repo_path, use_checkpoint=True, use_cache=True):
    ...
    last_print_count = 0
    PRINT_EVERY = 100  # Print progress every 100 commits
    ...
    print(f"  Processed {count} commits successfully" + " " * 20)
        ...
        if len(ts) >= 3:
            ...
            if gaps:
                ...
                if avg_gap > 0:
                    ...
                    # Cap burst with log transform to prevent extreme values
